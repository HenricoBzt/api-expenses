...
from app.crud.expenses_crud import create_expense, update_expenses, delete_expense
...
from app.models import UserModel, StatusType, ExpensesModel


async def create_expense_and_update_income(
    session: AsyncSession, current_user: UserModel, expense_data: ExpenseCreate
):
    try:
        ...
        if monthly_income and expense_data.status == StatusType.PAGO:
            ...
            await update_monthlyincome(
                session, current_user, monthly_income.id, monthly_income_update
            )
        return expense
...
async def update_expense_and_update_income(
    ...
    try:
        stmt = select(ExpensesModel).where(
            ExpensesModel.id == expense_id, ExpensesModel.user_id == current_user.id
        )
        ...
        if not monthly_income:
            return await update_expenses(
                session, current_user, expense_id, expense_data
            )

        if obj_expense.status != expense_data.status:
            if (
                obj_expense.status == StatusType.PAGO
                and expense_data.status != StatusType.PAGO
            ):
            ...
            obj_expense.status == StatusType.PAGO
            and expense_data.status == StatusType.PAGO
            and obj_expense.amount != expense_data.amount
        ):
        ...
        updated_expense = await update_expenses(
            session, current_user, expense_id, expense_data
        )
