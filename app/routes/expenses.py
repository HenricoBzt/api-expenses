...
async def exp_create(
    session: T_asyncsession, current_user: T_current_user, expense: ExpenseCreate
):
...
async def exp_list_by_id(
    expense_id: int, session: T_asyncsession, current_user: T_current_user
):
    stmt = select(ExpensePublic).where(
        ExpensePublic.id == expense_id, ExpensePublic.user_id == current_user.id
    )
...
async def exp_delete(
    ...
    return await delete_expense_and_update_income(session, current_user, expense_id)
